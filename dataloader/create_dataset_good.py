...
def check_file(file):
    if 'result' not in file.split('/')[-1]:
        with open(file, 'rb') as f:
            content = pkl.load(f)
        if content['finished'] and len(content['action'][0]) <= 150:
            return True
    return False
...
def build_dataset(path_str):
    ...
    for agent_folder in agent_folders:
        ...
        valid = p_map(check_file, files)
        
        for it, file in enumerate(tqdm(files)):
            if valid[it]:
...
if __name__ == '__main__':
    ...
    dataset_name = 'dataset_graph_full_150step'
    
    #train_path = f'{dir_script}/../dataset_episodes/large_data_toy/train_env_task_set_100_full/*'
    train_path =  f'{dir_script}/../dataset_episodes_noscratch/data_structured/train_env_task_set_150_full_task.all/*'
    test_path =  f'{dir_script}/../dataset_episodes_noscratch/data_structured/test_env_task_set_60_full_task.all/*'

    #dict_train = build_dataset(train_path)
    print("Building...")
    dict_train = build_dataset(train_path)
    with open(f'{dir_script}/../dataset/{dataset_name}_larger_train.pkl', 'wb+') as f:
