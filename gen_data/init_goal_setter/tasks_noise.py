...
def cleanup_graph(init_goal_manager, graph, start):
    ...
    container_ids_clean = [node for node in graph['nodes'] if node['class_name'] in objects_clean]
    ...
    ids_obj = [node for node in graph['nodes'] if node['class_name'] in objects_grab_clean]
