...
def inference(
    ...
    for it, data_item in enumerate(data_loader):
        ...
        if it < args['test']['num_iters']:
            ...
            metrics_item_tstep, metrics_item = update_metrics_recall_prec(metric_dict, args, predicted_mask, predicted_graph, gt['gt_mask'], gt['gt_task'], misc)
...
def evaluate(
    ...
    info_log = {
        'losses': {
            'total': metric_dict['losses'].avg,
            'task': metric_dict['losses_task'].avg,
            'mask': metric_dict['losses_mask'].avg,
        },
        'accuracy': {
            'change_prec': metric_dict['prec_change'].avg,
            'change_recall': metric_dict['recall_change'].avg,
            'task_accuracy': metric_dict['task_accuracy'].avg,
            'task_accuracy_pos': metric_dict['task_accuracy_pos'].avg
...
def update_metrics_recall_prec(metric_dict, args, gt, predicted_edge, changed_edge, var, misc):
    return None, None
...
def compute_metrics_change(
    ...
    pos_change = pred_mask_task.sum(-1) + 1e-9
    ...
    change_prec = tp_edge_change / change_avg_pos
