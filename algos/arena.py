...
class Arena:
    ...
    def get_actions(self, obs, action_space=None, agent_id=None):
        ...
        for it, agent in enumerate(self.agents):
            ...
            if agent.agent_type == 'MCTS':
                ...
                dict_actions[it], dict_info[it] = agent.get_action(
                    obs[it],
                    self.env.task_goal[it] if it == 0 else self.task_goal[it],
                    opponent_subgoal,
                )
                ...
                dict_actions[it], dict_info[it] = agent.get_action(
                    obs[it],
                    self.env.goal_spec if it == 0 else self.task_goal[it],
    ...
    def rollout(self, logging=False, record=False):
        ...
        while not done and nb_steps < self.max_episode_length:
            ...
            if logging:
                node_id = [
                    node['bounding_box'] for node in obs[0]['nodes'] if node['id'] == 1
                ][0]
                edges_char = [
                    (
                        id2node[edge['to_id']]['class_name'],
            ...
            entropy_action.append(
                -((agent_info[0]['probs'][0] + 1e-9).log() * agent_info[0]['probs'][0])
                .sum()
                .item()
            )
            entropy_object.append(
                -((agent_info[0]['probs'][1] + 1e-9).log() * agent_info[0]['probs'][1])
                .sum()
                .item()
            )
            ...
            for agent_id in range(self.num_agents):
                if self.agents[agent_id].agent_type == 'RL':
                    ...
                    policy = [
                        log_prob.data for log_prob in agent_info[agent_id]['probs']
                    ]
