...
def main(cfg: DictConfig):
    ...
    if not args.debug:
        cachedir = f"{get_original_cwd()}/outputs/helping_action_freq_fastwalk_r{int(args.reset_steps)}_{int(args.small_set)}_{args.num_tries}_ip{int(args.inv_plan)}_{network_name}_{args.num_samples}_{args.alpha}_{args.beta}_{args.lam}"
    else:
        cachedir = f"{get_original_cwd()}/outputs/debug_helping_action_freq_fastwalk_r{int(args.reset_steps)}_{int(args.small_set)}_{args.num_tries}_ip{int(args.inv_plan)}_{network_name}_{args.num_samples}_{args.alpha}_{args.beta}_{args.lam}"
            ...
            try:
                ...
                while steps < max_steps and not early_stopping:
                    ...
                    if len(task_result) == 0:
                        ...
                        res = manager.dict()
                        num_goals = len(proposals)
                        curr_pred_goals = [
                            proposal["pred"] for pred_id, proposal in proposals.items()
                        ]
                        if num_processes == 0:
