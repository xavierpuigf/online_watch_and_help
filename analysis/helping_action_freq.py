...
def main(cfg: DictConfig):
    ...
    for gt_path in gt_p:
        ...
        try:
            ...
            (prev_obs, reward, done, infos) = arena.step_given_action({0: actions[0]})
            ...
            while steps < max_steps:
                ...
                selected_actions, info = arena.get_actions(
                    curr_obs, length_plan=10, must_replan=[True], agent_id=0
                )
                # get helper action
                action_freq = {}
                for pred_id, pred_graph in enumerate(pred['pred_graph']):
                    ...
                    actions, info = arena.get_actions(
                        curr_obs, length_plan=10, must_replan=[True], agent_id=1
                    )
